...
function splitFirst(str: string, separator: string): [string, string] {
  let [first, ...rest] = str.split(separator);
  const restJoined = rest.join(separator);
...
type StringDifferenceArray = [number, string][];

function getStringDifferenceArray(
  str: string,
  stringDifference: string[],
  noMatchNumber: number,
  answerArray: StringDifferenceArray = [],
): StringDifferenceArray {
...
function _getStringDifferenceArrays(targetString: string, inputString: string): [StringDifferenceArray, StringDifferenceArray] | null {
...
function getStringDifferenceArrays(targetString: string, inputString: string): [StringDifferenceArray, StringDifferenceArray] {
  const diffArray = diffChars(targetString, inputString, { ignoreCase: true });

  const targetArray: StringDifferenceArray = [];
  const inputArray: StringDifferenceArray = [];
...
export type { StringDifferenceArray }; 